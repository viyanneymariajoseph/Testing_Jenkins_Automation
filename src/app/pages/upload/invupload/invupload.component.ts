...
export class InvuploadComponent implements OnInit {
  ...
  downloadFilestatus(strmsg: any) {
    const filename = "Inventory upload status";
 ...
 for (let i = 1; i < csvRecordsArr.length; i++) {  
   ...
   if (!carton_no || carton_no.trim() === ''|| carton_no === 'NULL' || carton_no === null || carton_no === undefined) {
    ...
    this._ColNameList[1] = "secured_unsecured";
    this._ColNameList[2] = "zone_name";
    this._ColNameList[3] = "applicant_name";
    this._ColNameList[4] = "scheme_name";
    this._ColNameList[5] = "smemel_product";
    this._ColNameList[6] = "disb_date";
    this._ColNameList[7] = "disb_number";
    this._ColNameList[8] = "total_disb_amt";
    this._ColNameList[9] = "pos";
    this._ColNameList[10] = "bal_disb_amt";
    this._ColNameList[11] = "carton_no";
    this._ColNameList[12] = "file_no";
    this._ColNameList[13] = "item_status";
  ...
  GetHeaderNames() {
    //  this._HeaderList = "";
    this._HeaderList =
      "lan_no,secured_unsecured,zone_name,applicant_name,scheme_name,smemel_product,disb_date,disb_number,total_disb_amt,pos,bal_disb_amt,carton_no,file_no,item_status";
  }
  ...
  prepareTableDataForCSV(tableData) {
    ...
    let tableHeader: any = [
      ...
      { field: "secured_unsecured", header: "Secured/Unsecured", index: 1 },
      { field: "zone_name", header: "Zone Nane", index: 2 },
      { field: "applicant_name", header: "APPLICATION NAME", index: 1 },
      { field: "scheme_name", header: "SCHEME NAME", index: 2 },
      { field: "smemel_product", header: "SMEMEL PRODUCT", index: 2 },
      { field: "disb_date", header: "DISB DATE", index: 2 },
      { field: "disb_number", header: "DISB NUMBER", index: 1 },
      { field: "total_disb_amt", header: "TOTAL DISB AMOUNT", index: 1 },
      { field: "pos", header: "POS", index: 2 },
      { field: "bal_disb_amt", header: "BAL DISB AMOUNT", index: 2 },
      { field: "carton_no", header: "Carton Number", index: 2 },
      { field: "file_no", header: "File Number", index: 1 },
  ...
  BindHeader(tableData, headerList) {
    ...
    let tableHeader: any = [
      { field: "srNo", header: "SR NO", index: 1 },
      { field: "lan_no", header: "Loan Account Number", index: 1 },
     // { field: "branch_name", header: "Branch_name", index: 1 },
      { field: "secured_unsecured", header: "Secured/Unsecured", index: 1 },
      { field: "zone_name", header: "Zone Nane", index: 2 },
      { field: "applicant_name", header: "APPLICATION NAME", index: 1 },
      { field: "scheme_name", header: "SCHEME NAME", index: 2 },
  ...
  GetPendingData() {
    ...
    this._onlineExamService
      .postData(this.DataUploadForm.value, apiUrl)
      // .pipe(first())
      .subscribe((data) => {
  ...
  GetColList() {
    ...
    for (let j = 0; j < this._ColList.length; j++) {
      this._HeaderColList +=
        this._ColList[j] + (j <= this._ColList.length - 2 ? "," : "");
      // headerArray.push(headers[j]);
    }
    ...
    this._StatusList.forEach((stat) => {
      for (let j = 0; j < this._ColList.length; j++) {
        this._HeaderColList +=
          stat[this._ColList[j]] + (j <= this._ColList.length - 2 ? "," : "");
        // headerArray.push(headers[j]);
      }
      this._HeaderColList += "\n";
    });
