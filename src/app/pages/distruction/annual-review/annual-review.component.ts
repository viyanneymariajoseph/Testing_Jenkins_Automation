  ...
  prepareTableData(tableData, headerList) {
    ...
    let tableHeader: any = [
      ...
      { field: "detailDocumentType", header: "DOCUMENT DETAILS", index: 1 },
            // { field: "isDestructionOrExtension", header: "IS DESTRUCTION", index: 1 },
      ...
      { field: "extensionDate", header: "EXTENSION DATE", index: 1 },
    ...
    tableData.forEach((el, index) => {
      formattedData.push({
        srNo: parseInt(index + 1),
            ...
            ReteionPeriod: el.ReteionPeriod != null ? el.ReteionPeriod + ' Year' : '',
...
downloadFile() {
  const filename = 'Annual_Review';
 
  // Capitalized header row
  let csvData =
    "CARTON NO,DEPARTMENT NAME,DEPARTMENT CODE,WAREHOUSE NAME,ITEM LOCATION,DOCUMENT TYPE,DOCUMENT DETAILS,INVENTORY BY,INVENTORY DATE,EXTENSION BY,EXTENSION DATE,RETENTION PERIOD,EXPIRE DATE\n";
  ...
  // Add data rows
  this.formattedData.forEach((row: any, i: number) => {
    csvData +=
      // `${i + 1},` +
      // `${row.batchId ?? ''},` +
        `${row.cartonNo ?? ''},` +
...
logDownloadActivity() {
  const payload = {
    pageName: 'Annual Review',
    activity: 'Download',
    activityDescription: 'User downloaded the annual review list.',
